var Radio = function()
{
  ...
  this.getFilters = function(req, res){
    ...
    self.getCities = function(callback){
      Media.aggregate(
        {$match: {toolId:self.toolId, "city": { $exists: 1}, isActive : 1}},
    ...
    self.getMusicLanguages = function(callback){
      Media.aggregate(
        {$match: {toolId:self.toolId, "musiclanguage": { $exists: 1}, isActive : 1}},
        {$group : { _id : '$musiclanguage', count : {$sum : 1}}},
        function(error, results) 
        {
          callback(error, results);
        }
    ...
    self.getStations = function(callback){
      Media.aggregate(
        {$match: {toolId:self.toolId, "station": { $exists: 1}, isActive : 1}},
        {$group : { _id : '$stations', count : {$sum : 1}}},
        function(error, results) 
        {
          callback(error, results);
        }
