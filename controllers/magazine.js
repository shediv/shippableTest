        ...
        async.series({
          ...
          medias : function(callback){
            ...
            for(var i= 0; i < result.medias.length; i++){
                ...
                var check = underscore.intersection(result.medias[i].keywords, ProductInfo[0].keywords);
                if(check.length > 0){
            ...
            var NonCatCS0 = [];
            if(!underscore.isEmpty(mediacategorybuckets[0])){
            ...
            if(!underscore.isEmpty(mediacategorybuckets[1])){
              for (var i = 0; i < NonCatCS0.length; i++) {        
                ...
                noncategorybuckets = underscore.clone(NonCatCS0);
              }
            ...
            if(!underscore.isEmpty(mediacategorybuckets[2])){
              for (var i = 0; i < NonCatCS1.length; i++) {        
                ...
                noncategorybuckets = underscore.clone(NonCatCS1);
              }

            //Find Medias that does not belong to any category with category 4
            var NonCatCS = [];        

            if(!underscore.isEmpty(mediacategorybuckets[3])){
              for (var i = 0; i < NonCatCS2.length; i++) {        
                ...
                noncategorybuckets = underscore.clone(NonCatCS2);
              }

            //shift the non category data to NonCatCS
            if(!NonCatCS.length > 0)
            {
              NonCatCS = underscore.clone(noncategorybuckets);
            }
            ...
            while (CountOfMedia < 9 && GeoMediaCount > 0) {
              ...
              if(YdataMediaCategoryBuckets1_Geo.length > 0){       
                FinalData.push(YdataMediaCategoryBuckets1_Geo[0]);
                CountOfMedia = CountOfMedia + 1;
              ...
              if(YdataMediaCategoryBuckets2_Geo.length > 0){       
                ...
                FinalData.push(YdataMediaCategoryBuckets2_Geo[0]);
                CountOfMedia = CountOfMedia + 1;
                GeoMediaCount = GeoMediaCount - 1;
              ...
              if(YdataMediaCategoryBuckets3_Geo.length > 0){       
                ...
                FinalData.push(YdataMediaCategoryBuckets3_Geo[0]);
                CountOfMedia = CountOfMedia + 1;
                GeoMediaCount = GeoMediaCount - 1;
              ...
              if(YdataMediaCategoryBuckets4_Geo.length > 0){                       
                FinalData.push(YdataMediaCategoryBuckets4_Geo[0]);
                CountOfMedia = CountOfMedia + 1;
                GeoMediaCount = GeoMediaCount - 1;
            ...
            if(NonCatCS){        
              for(var i= 0; i < NonCatCS.length; i++){            
                if(underscore.intersection(NonCatCS[i].geography, "55bd4a078ead0e5a288b4567")){
            ...
            if(mediaNonCategoryBuckets_GeoAllIndia.length > 0){        
              for(var i= 0; i < mediaNonCategoryBuckets_GeoAllIndia.length; i++){
                if(mediaNonCategoryBuckets_GeoAllIndia[i].IRS == 'Yes'){
...
function formatData(FinalData) {

  var magazine = [];

  for(var i= 0; i < FinalData.length; i++){
    var tmp = [];
    tmp['_id'] = FinalData[i]._id;
    tmp['categoryId'] = FinalData[i].categoryId;
    tmp['urlSlug'] = FinalData[i].urlSlug;
    tmp['thumbnail'] = FinalData[i].thumbnail;
    tmp['attributes'] = FinalData[i].attributes;
    tmp['IRS'] = FinalData[i].IRS;
